...
function doRegister() {
  ...
  var role = (STATE.regRole === "partner") ? "partner" : "developer";
  var emailKey = email.toLowerCase();

  /* Open registration: the user's selected role is honored directly.
     No allowlist gate — anyone may register as a developer or a partner. */
  auth.createUserWithEmailAndPassword(email, pass)
    .then(function (cred) {
      return db.collection(COL_PROFILES).doc(cred.user.uid).set({
        name: name, org: org, email: email, emailLower: emailKey, role: role,
        allowlisted: false,
  ...
  auth.signInWithPopup(provider)
    .then(function (result) {
      ...
      return ref.get().then(function (snap) {
        ...
        /* Open registration: honor the role selected on the form
           (defaults to developer). No allowlist gate. */
...
function subscribeProjects() {
  ...
  if (STATE.role === "developer") {
    ...
    q = db.collection(COL_PROJECTS).where("status", "in", ["open", "offered"]);
  }
...
function openDetail(pid) {
  ...
  db.collection(COL_PROJECTS).doc(pid).get().then(function (snap) {
    ...
    var offersP = db.collection(COL_PROJECTS).doc(pid).collection("offers").get();
    var inqP = db.collection(COL_PROJECTS).doc(pid).collection("inquiries").orderBy("createdAt", "asc").get();
